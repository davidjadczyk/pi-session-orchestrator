...
- ODD-2 validation evidence: `node --test --import tsx test/package-readiness.test.ts` passed 10/10; `npm ci --ignore-scripts --registry=https://registry.npmjs.org/` cleanly installed 175 packages without private credentials; `npm test` passed 48/48; `npm run build` passed; `npm run verify-pack` passed; `npm pack --dry-run --json --ignore-scripts` produced the expected 18-file package with no bundled dependencies.
...
- Confirmed CI failure: remote main run `35701928845` failed deterministically at `npm ci` with `E401`; the public GitHub runner had no private Artifactory credentials.
- Root cause: `package-lock.json` contained 34 package tarball URLs under `lht.app.lufthansa.com`, while `.github/workflows/ci.yml` did not explicitly select the public npm registry. The lockfile also contained 164 public npm URLs.
- Remediation evidence: the lockfile was normalized with `npm install --package-lock-only --ignore-scripts --registry=https://registry.npmjs.org/`, then its 34 private registry URLs were normalized to equivalent `registry.npmjs.org` tarball URLs without changing package versions or dependency declarations; CI setup-node now declares `registry-url: https://registry.npmjs.org`; the readiness test rejects every non-public lockfile `resolved` host and verifies registry selection precedes `npm ci`.
